...
The JavaScript function receives explicit `(rev, memory)` arguments. It can read the latest game state, click at absolute screen coordinates, sleep asynchronously, and retain state between invocations. Console commands control which script is loaded and whether it is running, paused, or stopped.
...
- `GameState`, shared by the UDP receiver and script runner through `Arc<std::sync::RwLock<GameState>>`;
- a UDP receiver task that parses valid telemetry and updates `GameState`;
- a console input task that parses lines into `ScriptCommand` values and sends them through an MPSC channel;
- a script runner that exclusively owns the current `AsyncRuntime`, `AsyncContext`, script function, `rev` API object, and `memory` object.

The standard-library lock is appropriate because each critical section only clones or replaces a small value and never crosses an await point. The script runner never holds the lock while evaluating JavaScript or awaiting a promise. `rev.state()` briefly acquires the lock, clones a snapshot, releases the lock, and then converts the snapshot into a JavaScript object.
...
The initial score and receive time are `None`, and the initial sequence is `0`. Every valid datagram replaces the score, records the current Unix time in milliseconds, and increments the sequence with saturating arithmetic. The score stays a string so the client never loses precision by converting it through `f64`.

Malformed UTF-8, malformed JSON, and payloads without a string `score` field leave the previous valid state unchanged. Unknown JSON fields are ignored. The receiver prints a concise warning for an invalid packet and continues. Failure to bind the socket or an unrecoverable receive error terminates the client so automation cannot continue indefinitely against permanently stale telemetry.
...
```javascript
(async (rev, memory) => {
    const game = rev.state();

    memory.ticks ??= 0;
    memory.ticks += 1;

    if (game.score !== null && memory.ticks === 10) {
...
The evaluated function is retained by the script runner and invoked repeatedly as `script(rev, memory)`. The return value may be synchronous or a promise; the runner awaits promise settlement before beginning the next invocation. After a completed or failed invocation, the runner waits 50 ms before invoking it again unless a lifecycle command is ready.
...
`rev` is one frozen host-owned object created with each fresh QuickJS context. Its API properties cannot be replaced by the script.

#### `rev.state()`

Returns a new object containing a snapshot of Rust game state:
...
Changing the returned object does not change Rust state.
...
`memory` and ordinary JavaScript globals survive normal invocations, script exceptions, pause, and resume. They are destroyed by load, reload, stop, or process exit.

## Script Lifecycle

The externally visible script states are `Running`, `Paused`, and `Stopped`.

All lifecycle commands are applied at a safe invocation boundary. If a command arrives while the script function is active, including while it awaits `rev.sleep`, the command remains queued until that invocation settles. There is no forced cancellation or execution timeout in this version. A script that never settles also prevents pending lifecycle commands from completing.
...
- Preserve the QuickJS runtime, context, script function, `rev`, `memory`, and JavaScript globals.
...
The UDP receiver and Rust `GameState` continue unchanged across pause, resume, reload, stop, and load. Resetting a JavaScript context never resets the latest telemetry state.
...
- Script file read errors, syntax errors, non-callable evaluation results, QuickJS initialization errors, and Enigo initialization errors fail that load operation and leave the runner `Stopped`.
...
- `client/src/state.rs`: `GameState`, shared-state alias, and snapshot conversion data.
- `client/src/udp.rs`: payload deserialization, loopback socket binding, receive loop, and state updates.
- `client/src/console.rs`: `ScriptCommand`, console-line parsing, and asynchronous stdin command forwarding.
- `client/src/script.rs`: QuickJS ownership, lifecycle state machine, script loading, repeated invocation, host API bindings, and Enigo integration.

No window-control module or direct Windows window API dependency is added.

## Implementation Tasks

### Task 1: Receive UDP telemetry and update shared state

Create the shared state and UDP modules, wire the receiver into `main`, and verify that valid packets update the latest snapshot while invalid packets preserve it. This task produces a stable Rust state interface for the script runner.

### Task 2: Run and control the JavaScript automation context

Create the console and script modules, expose `(rev, memory)`, implement repeated invocation and the five console commands, and wire the runner into `main`. This task consumes the shared state produced by Task 1.
...
- `rev.state()` observing later Rust state updates;
...
- every lifecycle transition and no-op transition;
